...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleErr(err)) => panic!("{err}"),
        Err(TicketNewError::DescriptionErr(_)) => {Ticket::new(title, "Description not provided".to_string(), status).unwrap()},
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if description.is_empty() {
            return Err(TicketNewError::DescriptionErr("Description empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionErr("Description long".to_string()));
        }
